...
for i in range(15667):
    prefix_train_pos = 'TRAIN_POS_' + str(i)
    #prefix_train_neg = 'TRAIN_NEG_' + str(i)
    #train_arrays[i] = model.docvecs[prefix_train_pos]
    train_arrays[10240 + i] = model.docvecs[prefix_train_pos]
    #train_labels[i] = 1
    train_labels[10240 + i] = 1
    
test_arrays = np.zeros((12828, 100))
test_labels = np.zeros(12828)
...
#text_train, text_test, y_train, y_test = train_test_split(text_train, y_train, test_size=0.33)
#train_arrays, test_arrays, train_labels, test_labels = train_test_split(train_arrays, train_labels, test_size=0.33)
...
X_train = hstack((X_train,train_arrays))
X_test = hstack((X_test,test_arrays))
...
pos = ["worst movi"]
print("NEG prediction: {}". format(lr.predict(hstack((vect.transform(pos),model.infer_vector(pos))))))

neg = ["good movie"]
print("POS prediction: {}". format(lr.predict(hstack((vect.transform(neg),model.infer_vector(neg))))))
